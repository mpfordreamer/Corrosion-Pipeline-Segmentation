...
async def save_prediction(prediction_type: PredictionType):
    global uploaded_image
    ...
    for pred_type in prediction_types:
        if pred_type == PredictionType.intersection:
            prediction_image = segment_intersection_function()  # No await needed
        elif pred_type == PredictionType.asset:
            prediction_image = segment_asset_function()  # No await needed
        elif pred_type == PredictionType.corrosion:
            prediction_image = segment_corrosion_function()  # No await needed
        else:
            raise HTTPException(status_code=400, detail="Invalid prediction type.")

        # Save the image to the folder
        filename = f"{pred_type.value}_{timestamp}.jpg"
        file_path = f'save_images/{filename}'

        # Save the predicted image
        with open(file_path, "wb") as f:
            f.write(prediction_image)  # This should now work since prediction_image is bytes

        saved_files.append(filename)

    return {"message": f"Images saved successfully: {', '.join(saved_files)}."}
